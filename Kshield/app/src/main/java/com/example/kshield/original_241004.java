...
public class original_241004 extends AppCompatActivity {
    ...
    private void goToNextLevel() {
        ...
        if (currentLevel < 10) {  // 최대 10레벨까지만 있다고 가정
            ...
            // 다음 레벨 페이지로 이동
            Intent intent = new Intent(original_241004.this, LevelActivity.class);
            intent.putExtra("level", nextLevel);
            startActivity(intent);
            finish();  // 현재 액티비티 종료
        } else {
            // 최대 레벨을 넘었을 경우, 메인 화면으로 돌아가기
            Intent intent = new Intent(original_241004.this, subpage1.class);
            startActivity(intent);
            finish();
        }
    }
